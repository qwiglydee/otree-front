...
class Slider extends Directive {
  ...
  onUpdate(event) {
    ...
    const affected = {
      img: changes.affects(this.imgref),
      val: changes.affects(this.valref),
      sts: changes.affects(this.stsref),
    };
    ...
    if (affected.sts) {
      this.valid = changes.pick(this.stsref);
    }

    if (affected.img || affected.val || affected.sts) {
      this.redraw();
    }
  }
  ...
  redraw() {
    ...
    const x = this.coords.x0 + this.value, y = this.coords.y0;
    ...
    this.draw.fillStyle = this.valid ? "rgb(0, 139, 0)" : "rgb(98, 0, 238)";
